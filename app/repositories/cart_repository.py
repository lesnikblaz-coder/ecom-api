...
async def delete(db: AsyncSession, obj: Any) -> None:
    await db.delete(obj)
    await db.flush()
...
async def cart_item_save(db: AsyncSession, cart_item: CartItem) -> CartItem | None:
    await create_or_save(db, cart_item)

    return await cart_item_get_by_id(db, cart_item.cart_id, cart_item.cart_item_id)
