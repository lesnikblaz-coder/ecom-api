...
def users_get(db: db_session, _: User = Depends(auth.require_admin)) -> Sequence[User]:
    return user_services.users_get(db)

@app.get("/users/{user_id}", response_model=schemas.UserResponse)
def user_get(user_id: int, db: db_session, _: User = Depends(auth.require_admin)) -> User:
    return user_services.user_get(db, user_id)

@app.put("/users/{user_id}", response_model=schemas.UserResponse)
def user_update(user_id: int, data: schemas.UserUpdate, db: db_session, _: User = Depends(auth.require_admin)) -> User:
    return user_services.user_update(db, user_id, data)

@app.delete("/users/{user_id}", status_code=204)
def user_delete(user_id: int, db: db_session, _: User = Depends(auth.require_admin)) -> None:
